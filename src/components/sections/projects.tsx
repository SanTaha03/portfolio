          ...
          {projects.map((project, index) => (
            <ScrollReveal key={index} delay={index * 0.1}>
              <TiltCard className="h-full">
                <div className="group h-full bg-card border border-border rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col">
                  {/* Image Placeholder */}
                  <div className={`h-48 w-full ${project.image} relative`}>
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/50 backdrop-blur-sm">
                      <Link 
                        href={project.link}
                        className="p-2 bg-white rounded-full text-black hover:bg-primary hover:text-white transition-colors mx-2 transform scale-0 group-hover:scale-100 transition-transform duration-300 delay-75"
                      >
                        <ExternalLink className="w-5 h-5" />
                      </Link>
                      <Link 
                        href={project.github}
                        className="p-2 bg-white rounded-full text-black hover:bg-primary hover:text-white transition-colors mx-2 transform scale-0 group-hover:scale-100 transition-transform duration-300 delay-150"
                      >
